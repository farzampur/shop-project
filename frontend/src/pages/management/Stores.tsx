...
export default function Stores() {
  ...
  return <Box dir="rtl"><Stack direction={{ xs: "column", sm: "row" }} className="page-header" sx={{ mb: 0, justifyContent: "space-between", gap: 1 }}><Box><Typography variant="h5" sx={{ fontWeight: 700 }}>مدیریت شعب</Typography><Typography variant="body2" color="text.secondary">اطلاعات و وضعیت شعبه‌های در دسترس</Typography></Box><Stack className="page-header-actions" direction="row" spacing={1}><Button startIcon={<RefreshIcon />} variant="outlined" onClick={() => void load()}>به‌روزرسانی</Button>{isSuperuser && <Button startIcon={<AddIcon />} variant="contained" onClick={openCreate}>شعبه جدید</Button>}</Stack></Stack>
