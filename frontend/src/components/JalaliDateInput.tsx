...
export default function JalaliDateInput({label,value,onChange,required=false,allowClear=false}:{label:string;value:string;onChange:(value:string)=>void;required?:boolean;allowClear?:boolean}){
 ...
 const days=jalaliMonthDays(view.y,view.m), first=(jalaliMonthFirstWeekday(view.y,view.m)+1)%7;
 ...
 return <>
  ...
  <Popover open={Boolean(anchor)} anchorEl={anchor} onClose={()=>setAnchor(null)} anchorOrigin={{vertical:"bottom",horizontal:"right"}} transformOrigin={{vertical:"top",horizontal:"right"}}>
   <Box sx={{p:2,width:360,maxWidth:"calc(100vw - 24px)",boxSizing:"border-box",dir:"rtl"}} dir="rtl"><Stack direction="row" sx={{alignItems:"center",justifyContent:"space-between"}}><IconButton onClick={()=>setView(v=>shiftJalali(v.y,v.m,-1))}><ChevronRightIcon/></IconButton><Typography sx={{fontWeight:700}}>{months[view.m-1]} {view.y}</Typography><IconButton onClick={()=>setView(v=>shiftJalali(v.y,v.m,1))}><ChevronLeftIcon/></IconButton></Stack>
    <Box sx={{display:"grid",gridTemplateColumns:"repeat(7,minmax(0,1fr))",gap:.5,mt:1,width:"100%"}}>{weekdays.map(w=><Typography key={w} align="center" variant="caption" sx={{fontWeight:700}}>{w}</Typography>)}{cells.map((d,i)=>d===null?<Box key={`e${i}`}/>:<Button key={d} size="small" onClick={()=>select(d)} variant={parsed?.jy===view.y&&parsed?.jm===view.m&&parsed?.jd===d?"contained":"text"} sx={{minWidth:0,width:"100%",px:0}}>{d.toLocaleString("fa-IR")}</Button>)}</Box>
