...
export type Daily={day:string;order_count:number;total_sales:string}; export type Monthly={month:string;order_count:number;total_sales:string};
export type PaymentMethod={method:string;method_name:string;amount:string;transaction_count:number}; export type TopProduct={product_id:number;product_name:string;total_quantity:string;total_sales:string};
export type Profit={total_sales:string;total_cost:string;total_profit:string}; export type Financial={sales:string;expenses:string;balance:string};
