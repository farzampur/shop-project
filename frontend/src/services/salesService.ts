...
export interface CartItem { id:number; product:number; product_name:string; purchase_price?:string; quantity:string; unit_price:string; discount_percent:string; discount_amount:string; final_unit_price:string; total_price_before_discount:string; total_discount_amount:string; total_price:string; }
...
export async function addCartItem(cartId:number,data:{product?:number;barcode?:string;quantity:string|number;discount_percent:string|number}):Promise<CartItem>{const r=await api.post<CartItem>(`/sales/carts/${cartId}/items/`,data);return r.data;}
