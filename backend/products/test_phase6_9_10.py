...
class PurchasePricingIntegrity610Tests(TestCase):
    def setUp(self):
        self.store = Store.objects.create(name="610 Store")
        ...
        self.product = Product.objects.create(category=self.category, name="610 Product", barcode="61001")
    ...
    def test_purchase_item_quantity_must_be_positive(self):
        self.assert_db_rejects(PurchaseItem, purchase=self.purchase, product=self.product, quantity=0, unit_price=10)

    def test_product_batch_prices_cannot_be_negative(self):
        item = PurchaseItem.objects.create(purchase=self.purchase, product=self.product, quantity=1, unit_price=10)
        self.assert_db_rejects(ProductBatch, purchase_item=item, product=self.product, store=self.store, quantity=1, remaining_quantity=1, purchase_price=-1, sale_price=10)

    def test_product_price_must_be_positive(self):
        self.assert_db_rejects(ProductPrice, product=self.product, store=self.store, amount=0, created_by=self.user)
